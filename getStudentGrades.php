<?php

//print_r($_POST); die();

$grade1 = $_POST['grade1'];
$grade2 = $_POST['grade2'];
$grade3 = $_POST['grade3'];
$grade4 = $_POST['grade4'];
$grade5 = $_POST['grade5'];

function getGrade($score)
{
    switch(true)
    {
        case ($score>=80 && $score<=100):
            return 'A';
            break;
        case ($score>=60 && $score<=79):
            return 'B';
            break;
        case ($score>=50 && $score<=59):
            return 'C';
            break;
        case ($score>=45 && $score<=49):
            return 'D';
            break;
        case ($score>=40 && $score<=44):
            return 'E';
            break;
        case ($score>=0 && $score<=39):
            return 'F';
            break;
        default:
            return 'Invalid';
    }
}

$grades=['A'=>0,'B'=>0,'C'=>0,'D'=>0,'E'=>0,'F'=>0,'Invalid'=>0];

// echo "before add to key D";
// print_r($grades);

// echo"</br>";

// $grades["D"]+=7;

// print_r($grades);
// echo "after add value to key D";
// die();


if(is_numeric($grade1) && ($grade1>=0 && $grade1<=100))
{
    //associate the grade with the count
    $grades[getGrade($grade1)]+=1;
}
else
{
    $grades['Invalid']+=1;
}


if(is_numeric($grade2) && ($grade2>=0 && $grade2<=100))
{

    $grades[getGrade($grade2)]+=1;
}
else
{
    $grades['Invalid']+=1;
}


if(is_numeric($grade3) && ($grade3>=0 && $grade3<=100))
{

    $grades[getGrade($grade3)]+=1;
}
else
{
    $grades['Invalid']+=1;
}


if(is_numeric($grade4) && ($grade4>=0 && $grade4<=100))
{
    $grades[getGrade($grade4)]+=1;
}
else
{
    $grades['Invalid']+=1;
}


if(is_numeric($grade5) && ($grade5>=0 && $grade5<=100))
{
    $grades[getGrade($grade5)]+=1;
}
else
{
    $grades['Invalid']+=1;
}


//redirect to from page with grade query strings
header('location:viewResultStats.php?A='.$grades['A'].'&B='.$grades['B'].'&C='.$grades['C'].'&D='.$grades['D']. '&E=' . $grades['E'].'&F='.$grades['F'].'&Invalid='.$grades['Invalid']);
exit();